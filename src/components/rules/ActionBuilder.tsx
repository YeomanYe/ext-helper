...
export function ActionBuilder({ actions, onChange }: ActionBuilderProps) {
  ...
  const disabledExtensions = actions
    ...
    .map((a) => a.targetId)
  const enabledGroups = actions.filter((a) => a.type === "enableGroup").map((a) => a.targetId)
  const disabledGroups = actions.filter((a) => a.type === "disableGroup").map((a) => a.targetId)
  ...
  const toggleExtensionEnable = (extId: string) => {
    if (enabledExtensions.includes(extId)) {
      ...
      const newActions = actions.filter(
        (a) => !(a.type === "disableExtension" && a.targetId === extId)
      )
  ...
  const toggleExtensionDisable = (extId: string) => {
    if (disabledExtensions.includes(extId)) {
      ...
      const newActions = actions.filter(
        (a) => !(a.type === "enableExtension" && a.targetId === extId)
      )
  ...
  const toggleGroupEnable = (groupId: string) => {
    if (enabledGroups.includes(groupId)) {
      ...
      const newActions = actions.filter(
        (a) => !(a.type === "disableGroup" && a.targetId === groupId)
      )
  ...
  const toggleGroupDisable = (groupId: string) => {
    if (disabledGroups.includes(groupId)) {
      ...
      const newActions = actions.filter(
        (a) => !(a.type === "enableGroup" && a.targetId === groupId)
      )
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-1 max-h-48 overflow-y-auto">
            {filteredGroups.map((group) => {
              const isSelected =
                enabledGroups.includes(group.id) || disabledGroups.includes(group.id)
              return (
                <div
                  key={group.id}
                  className={cn(
                ...
                >
                  ...
                  {group.iconUrl ? (
                    <img
                      src={group.iconUrl}
                      className="h-5 w-5 object-cover flex-shrink-0"
                      alt=""
                    />
                  ) : (
                    <div
