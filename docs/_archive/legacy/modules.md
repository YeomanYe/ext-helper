...
│               │ ExtPicker     │ ConditionBldr │ ExtensionsTab            │
│               │               │ ActionBuilder │ PopupPage (Tab 路由)     │
│               │               │ RuleBadges    │ ErrorBoundary            │
├───────────────┴───────────────┴───────────────┴──────────────────────────┤
│                            Services Layer                                │
...
interface ExtensionStore {
  ...
  loading: boolean
  error: string | null
  filter: FilterType          // 'all' | 'enabled' | 'disabled' | 'favorites' | 'in-group' | 'not-in-group' | 'in-rule' | 'no-rule' | 'in-any-group' | 'no-any-group'
  searchQuery: string
...
│   ├── Tooltip.tsx             # 悬浮提示
│   ├── ErrorBoundary.tsx       # React 错误边界（包裹 Tab 内容）
│   └── index.ts
│
├── extension/
...
│
├── popup/
│   ├── Header.tsx               # Header + SearchBar + QuickFilters + Footer + 过滤器常量
│   ├── ExtensionsActionsMenu.tsx # Actions 下拉菜单
│   ├── BisectBanner.tsx         # Bisect 状态横幅
│   ├── ExtensionsTab.tsx        # Extensions Tab 主内容（从 PopupPage 抽取）
│   └── index.ts
│
└── PopupPage.tsx                # 主页面入口（Tab 路由 + Header + Footer + ErrorBoundary）
```
...
```
       ...
       └────────┬─────────────┘                  │
                ...
                ▼                                ▼
     ┌────────────────────────┐         ┌──────────────────┐
     │ extensionStore         │         │    rulesRepo     │
     │ groupStore             │         │ (sync storage)   │
     │ uiStore                │         └────────┬─────────┘
     └──────────┬─────────────┘                  │
                │                                │
                ▼                                ▼
        ┌────────────────────────────────────────────────┐
        │             Repository Layer                   │
        │ extensionsRepo (local) │ groupsRepo (sync) │   │
        │ preferencesRepo (local)                        │
        └──────────────────────┬─────────────────────────┘
                               │
                   ┌───────────┴───────────┐
                   ▼                       ▼
