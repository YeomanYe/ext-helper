...
- [x] `site-applicable-extensions` 当前网站可用扩展识别 — 在当前网站上下文中，展示已安装扩展里适用于此站点的扩展，并推荐未安装的相关扩展 (通过悬浮触发按钮 + 侧边发现面板实现；适用性判断基于 AI 分析而非使用记录)
- [x] `cloud-extension-recommendation` 云端推荐可用扩展 — 查询云端目录，推荐可以用在当前网站上的扩展 (安装用户无需登录，每天可查询 3 次；服务部署在 Cloudflare Workers，并用 KV 按匿名安装 ID 计数)
